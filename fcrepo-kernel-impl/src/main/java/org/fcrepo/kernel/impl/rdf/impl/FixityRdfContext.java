...
public class FixityRdfContext extends NodeRdfContext {
    ...
    public FixityRdfContext(final FedoraResource resource,
        ...
        join(Sequences.flatMap(blobs, new Function1<FixityResult, RdfStream>() {

            @Override
            public RdfStream call(final FixityResult blob) throws RepositoryException {
                final com.hp.hpl.jena.graph.Node resultSubject = getTransientFixitySubject();
                final RdfStream b = new RdfStream();
                b.append(create(idTranslator.reverse().convert(resource).asNode(),
                        HAS_FIXITY_RESULT.asNode(), resultSubject));
                b.append(create(resultSubject, RDF.type.asNode(), FIXITY_TYPE.asNode()));
                ...
                for (final FixityResult.FixityState state : blob.getStatus(size, digest)) {
                    b.append(create(resultSubject, HAS_FIXITY_STATE
                            .asNode(), createLiteral(state
                            .toString())));
                }
                final String checksum =
                        blob.getComputedChecksum().toString();
                b.append(create(resultSubject, HAS_MESSAGE_DIGEST
                        .asNode(), createURI(checksum)));
                b.append(create(resultSubject, HAS_SIZE.asNode(),
                        createTypedLiteral(
                                blob.getComputedSize())
                                .asNode()));
                b.append(create(resultSubject, HAS_CONTENT_LOCATION.asNode(),
                        contentLocation));
                b.append(create(contentLocation,
                        RDF.type.asNode(),
